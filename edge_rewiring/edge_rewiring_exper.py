...
def loop_Alg1_expr(index, iter, times, min_size, max_size, latex_list_one, latex_list_two):
    ...
    for j in range(times):
        ...
        for i in range(iter):
            ...
            # Check power law for this iteration's hypergraph
            follows, pl_stats = power_law.has_powerlaw(H)
            powerlaw_results.append(follows)
            # Updates all values each time, checking the if statements
            num_max_hyperedges += stats["num_maximal_hyperedge"] #should count only once
            num_missing_subfaces += stats["num_missing_subface"] #number of missing subfaces in a particular region 
            ...
            print(colored(datasets[index], 'blue'), stats, f"Power-law fit: {follows}, alpha={pl_stats['alpha'] if pl_stats else 'N/A'}")
    ...
    failure_rate = round((avg_failures / iter), 5)
    avg_num_missing_subfaces = total_num_missing_subfaces / times
    avg_powerlaw = sum(powerlaw_results) / len(powerlaw_results) if powerlaw_results else 0 
    ...
    print( Fore.LIGHTGREEN_EX + str(datasets[index]) + ": \n" + 
          ...
          f" average power-law fir success = {avg_powerlaw}\n")
...
if __name__ == "__main__":
    ...
    threads = []
    for i in range(10):
        thread = threading.Thread(target=loop_Alg1_expr, args=(i, 2, 2, min_size, max_size, latex_list_one, latex_list_two))
