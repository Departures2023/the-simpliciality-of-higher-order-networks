...
def Construct_New_Graph(index, iter, min_size, max_size, total):
    ...
    og_edges = {frozenset(G.edges.members(edge_id)) for edge_id in G.edges}
    ...
    for i in range(iter):
        ...
        jaccard_index.append((i + 1, jaccard))
...
def process_dataset (index, trials, rewiring_times, min_size, max_size, latex_list_one, 
    ...
    Construct_New_Graph(index, rewiring_times, min_size, max_size, total)        
...
if __name__ == "__main__":
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'jaccard_vs_rewiring_{datasets[begin_dataset]}.png') 
