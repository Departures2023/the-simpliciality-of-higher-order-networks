...
def possible_combinations(num_node, min_size=2, max_size=None):
    """
    Calculate the total number of possible hyperedges (combinations of nodes)
    in a hypergraph, given the number of nodes and a size range for the hyperedges.
    ...
    min_size : int, optional (default=2)
        Minimum size of a hyperedge (must be at least 2).
    max_size : int or None, optional (default=None)
        Maximum size of a hyperedge where max_size ≤ num_node. 
        If None, defaults to num_node.

    Returns:
    -------
    int
        Total number of possible combinations (hyperedges) where each hyperedge
        size is between min_size and max_size (inclusive).
    """
...
def combination_to_size(C):
    """ 
    Incrementally increases the number of nodes and calculates the total 
    number of possible combinations of those nodes, considering hyperedges of size 
    ranging from 2 up to the number of nodes. It stops when the number of possible 
    combinations meets or exceeds `C`.

    Parameters:
    ----------
    C : int
        Target number of combinations
...
def generate_C_distribution(min_size, max_size, C_avg, num_max_hyperedge, target_sum):
    ...
    min_size : int
        Minimum size of a hyperedge (must be at least 2).
    max_size : int 
        The maximum size of edges of the generated graph where
        max size ≤ num node
    C_avg : float
        The average number of simplicial edges per maximal hyperedge.
    num_max_hyperedge: int
        The number of maximal hyperedges of the generated graph.
    target_sum : int
        The target number of simplicial combinations  

    Returns:
    -------
    np.ndarray
        An integer-valued array of simplicial combinations in each maximal hyperedge
    """
